...
we are going to return more vivi answers for question compared with the state-of-art Qna search engine.<br>
Now google and bing provide user exact answers or specific passage which contains answer.
However we think this is not good enough, we cant make more vivid results. For example ![](./image/example1.png).
I think it will be clear and convient for users to catch the key points.

### Plans

1. Now it supports for searching on [MS Marco](http://www.msmarco.org/) data set more than 80K QnA dataset.
    * the backen search engine is provided by Elastic Search.
2. Next I will introduce knowledge graph and NLI(natrucal language inference) to build relation on the given passages and understand some week logic.
3. I will try to split the query set, from some easy segment, to show some vivid answers. like numeric segment.
4. TBD
